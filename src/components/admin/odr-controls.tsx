...
// encore envoyés (+ liste des dossiers sans n°), aperçu PDF de la lettre remplie,
// et envoi Front (1 mail/assureur, PDF joint) qui passe les dossiers en « ODR envoyées ».
...
function PartnerRow({ p }: { p: OdrPartnerSummary }) {
  ...
  async function send() {
    ...
    if (!window.confirm(`Envoyer l'ordre de remplacement à ${p.label} (${to}) pour ${p.ready} contrat${p.ready > 1 ? "s" : ""} ?\nLes dossiers passeront en « ODR envoyées ».`)) return;
    setSending(true);
    try {
      const res = await fetch("/api/odr/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ partner: p.key, to: to.trim() }),
      });
    ...
    <div style={{ border: "1px solid #E8E8EC", borderRadius: 10, padding: "12px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          <a href={`/api/odr/export?partner=${p.key}&kind=ready`} style={{ textDecoration: "none", pointerEvents: noReady ? "none" : "auto", opacity: noReady ? 0.4 : 1 }}>
            <Button variant="outline" size="sm" className="gap-1.5" disabled={noReady}>
          ...
          <a href={`/api/odr/pdf?partner=${p.key}`} target="_blank" rel="noreferrer" style={{ textDecoration: "none", pointerEvents: noReady ? "none" : "auto", opacity: noReady ? 0.4 : 1 }}>
            <Button variant="outline" size="sm" className="gap-1.5" disabled={noReady}>
      ...
      {open && !noReady && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, paddingTop: 10, borderTop: "1px dashed #E8E8EC", flexWrap: "wrap" }}>
          <input
            type="email"
          ...
          <Button size="sm" onClick={send} disabled={sending} className="gap-1.5">
            ...
            {sending ? "Envoi…" : `Envoyer les ${p.ready} contrats`}
          </Button>
          <span style={{ fontSize: 12, color: "#A2A1AF" }}>PDF joint · passage en « ODR envoyées »</span>
        </div>
      )}
    </div>
  );
